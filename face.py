...
for facx in face:
    ...
    th2 = cv2.adaptiveThreshold(img,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,41,18)
    ...
    for i in range(height):
        for j in range(width):
            out[i,j] = hsv[offset+i, offset+j] if img[i,j] > 3 else [hsv[offset+i,offset+j][0],hsv[offset+i,offset+j][1], hsv[offset+i,offset+j][2]*0.5]#[img[i,j], img[i,j], img[i,j]] 
